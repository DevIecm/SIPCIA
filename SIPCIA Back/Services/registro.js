...
//consulta por seccion electoral
router.get("/getbyseccion", Midleware.verifyToken, async (req, res) => {
    try {
        const { seccion_electoral } = req.query; //pibote por medio de la seccion electoral
        ...
        const result = await pool.request()
            ...
            .query(`
                ...
                dt.demarcacion_territorial,
                cd.direccion_distrital
                from cat_seccion as r
                join demarcacion_territorial as dt on r.demarcacion_territorial = dt.id 
                join cat_distrito as cd on r.distrito_local = cd.id 
                where r.seccion_electoral = @seccion_electoral;`
            )
        if (result.recordset.length > 0) {
            return res.status(200).json({
                getbyseccion: result.recordset
            });
        } else {
            return res.status(404).json({ message: "No se encontraron registros", code: 100})
        }
