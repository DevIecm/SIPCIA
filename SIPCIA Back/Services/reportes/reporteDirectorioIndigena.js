...
router.get("/reporteDirectorioIndig", Midleware.verifyToken, async (req, res) => {
            const { tipo_comunidad,  distrito_electoral} = req.query;

            if(!tipo_comunidad || !distrito_electoral){
    ...
    .input('distrito_electoral', sql.Int, distrito_electoral)
      ...
      WHERE r.distrito_electoral = @distrito_electoral AND r.comunidad = @tipo_comunidad;`);
...
router.get("/reporteInstancias", Midleware.verifyToken, async (req, res) => {
    ...
    .input('distrito_electoral', sql.Int, distrito_electoral)
        .query(`SELECT
                ...
                COUNT(DISTINCT r.pueblo_originario) AS pueblos_originarios,
                COUNT(DISTINCT r.pueblo_pbl) AS pueblos,
                COUNT(DISTINCT r.barrio_pbl) AS barrios,
                COUNT(DISTINCT r.comunidad_pbl) AS comunidades_indigenas,
                COUNT(DISTINCT r.unidad_territorial_pbl) AS ut,
                COUNT(DISTINCT r.otro_pbl) AS otros,
                (
                    COUNT(DISTINCT r.pueblo_originario) +
                    COUNT(DISTINCT r.pueblo_pbl) +
                    COUNT(DISTINCT r.barrio_pbl) +
                    COUNT(DISTINCT r.comunidad_pbl) +
                    COUNT(DISTINCT r.unidad_territorial_pbl) +
                    COUNT(DISTINCT r.otro_pbl)
                ) AS total
                FROM registro r 
                join demarcacion_territorial dt on r.demarcacion = dt.id 
                join cat_distrito cd on r.distrito_electoral = cd.id 
    ...
    const result2 = await pool.request()
      ...
      .query(`
            SELECT
            dt.demarcacion_territorial, 
            COUNT(DISTINCT r.pueblo_afro) AS pueblos,
            COUNT(DISTINCT r.comunidad_afro) AS comunidad,
            COUNT(DISTINCT r.organizacion_afro) AS organizacion_afro,
            COUNT(DISTINCT r.persona_relevante_afro) AS persona_relevante_afro,
            COUNT(DISTINCT r.otro_afro) AS otros,
            (
                COUNT(DISTINCT r.pueblo_afro) +
                COUNT(DISTINCT r.comunidad_afro) +
                COUNT(DISTINCT r.organizacion_afro) +
                COUNT(DISTINCT r.persona_relevante_afro) +
                COUNT(DISTINCT r.otro_afro)
            ) AS total
            FROM registro r 
            join demarcacion_territorial dt on r.demarcacion = dt.id 
            join cat_distrito cd on r.distrito_electoral = cd.id
        WHERE r.distrito_electoral = @distrito_electoral 
          AND r.comunidad = 2
  ...
  const headers1 = [
  "Demarcación", "Pueblos Originarios", "Pueblos", "Barrios", 
  "Comunidades Indígenas", "UT", "Otros", "Total"
  ];

  worksheet.addRow(headers1);
  worksheet.columns = headers1.map(() => ({ width: 25 }));
    ...
    tabla1.forEach(row => {
      worksheet.addRow([
        ...
        row.pueblos_originarios,
        row.pueblos,
        row.barrios,
        row.comunidades_indigenas,
        row.ut,
        row.otros,
        row.total
      ]);
  ...
  worksheet.mergeCells(`C${startRow}:F${startRow}`); 
  const tituAfro = worksheet.getCell(`C${startRow}`);
    tituAfro.value =
    ...
    tituAfro.font = { size: 12, bold: true };
    tituAfro.alignment = { 
      vertical: "middle", 
      horizontal: "center", 
      wrapText: true 
    ...
    worksheet.addRow([
    "Demarcación", "Pueblos", 
    "Comunidades", "Organizaciones", "Personas Relevantes", "Otros", "Total"
    ]).eachCell((cell) => {
      cell.font = { bold: true, size: 11 };
