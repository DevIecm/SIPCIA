...
export class DocumentosIndigenas implements OnInit{
  ...
  search(): void {
    if(this.activeTab === 'home') {
      ...
      this.dataTableDN = this.allDatableDN.filter((val) => {
      ...
      this.dataTableDA = this.allDatableDN.filter((val) => {
        const id = (val.id ?? '').toString().toLowerCase().trim();
        const nombre_documento = (val.nombre_documento ?? '').toString().toLowerCase().trim();
        const fecha_carga = (val.fecha_carga ?? '').toLowerCase().trim();
  ...
  descargar(){
    this.docService.descargarDocNorma("1758128882717-purebaComunidadIndigena.pdf").subscribe({
      next: (blob) => {
    ...
    this.reporteService.getRegisterfichaTecnicaTabla(this.area_adscripcion, this.tokenSesion).subscribe({
      next: (data) => {
        if(data.getFichasInd.length > 0) {
          ...
          this.allDatableR = data.getFichasInd;
        } else {
          Swal.fire("No se encontraron registros");
        }        
      },
