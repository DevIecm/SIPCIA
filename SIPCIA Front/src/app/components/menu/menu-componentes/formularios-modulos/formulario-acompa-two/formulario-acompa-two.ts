...
@Component({
  selector: 'app-formulario-acompa-two',
  imports: [
    CommonModule,
    FormsModule,
    ReactiveFormsModule,
  ...
  providers: [DatePipe],
  templateUrl: './formulario-acompa-two.html',
  styleUrl: './formulario-acompa-two.css'
})

export class FormularioAcompaTwo {
      ...
      if(this.idRegistroC) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            formData.append("distrito_electoral", this.formularioRegistro.get('direccion')?.value || null);
  ...
  ngOnInit() {
    ...
    this.catalogo_direccion();
  }

  onChangeDireccion(){
    this.catalogo_demarcacion(this.formularioRegistro?.get('direccion')?.value);
    this.catalogo_pueblor(this.formularioRegistro?.get('direccion')?.value);
    this.catalogo_pueblos(this.formularioRegistro?.get('direccion')?.value);
    this.catalogo_barrios(this.formularioRegistro?.get('direccion')?.value);
    this.catalogo_unidad_territorial(this.formularioRegistro?.get('direccion')?.value);
    this.catalogo_comunidad(this.formularioRegistro?.get('direccion')?.value);      
  }

  catalogo_unidad_territorial(id: number) {
    this.catalogos.getCatalogos(id, "cat_unidad_territorial", this.tokenSesion).subscribe({
      next: (data) => {
  ...
  catalogo_pueblos(id: number) {
    this.catalogos.getCatalogos(id, "cat_pueblos", this.tokenSesion).subscribe({
      next: (data) => {
  ...
  catalogo_pueblor(id: number) {
    this.catalogos.getCatalogos(id, "cat_pueblos_originarios", this.tokenSesion).subscribe({
      next: (data) => {
    ...
    this.catalogos.getCatalogosD("cat_distrito", this.tokenSesion).subscribe({
      next: (data) => {
        if(data.cat_distrito.length > 0) {
          this.catalogoDireccion = data.cat_distrito;
        }
      }, error: (err) => {
        if(err.error.code === 160) {
          this.service.cerrarSesionByToken();
  ...
  catalogo_barrios(id: number) {
    this.catalogos.getCatalogos(id, "cat_barrios", this.tokenSesion).subscribe({
      next: (data) => {
  ...
  catalogo_comunidad(id: number) {
    this.catalogos.getCatalogos(id, "cat_comunidad", this.tokenSesion).subscribe({
      next: (data) => {
        ...
        next: (data) => {
          ...
          if(data.getRegistroInstituciones.length > 0) {
            ...
            const datosFormularioCompletos = {
              direccion: this.infoUpdate.distrito_electoral,
  ...
  catalogo_demarcacion(id: number) {
    this.catalogos.getCatalogos(id, "cat_demarcacion_territorial", this.tokenSesion).subscribe({
      next: (data) => {
  ...
  eliminaRegistro() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.delService.delSecondPointTenStep(this.idRegistro, this.tokenSesion).subscribe({
          next: (data) => {
            if(data.code === 200) {
              Swal.fire({
